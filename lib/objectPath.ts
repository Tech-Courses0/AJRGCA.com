 ...
 * (e.g. "hero.subtitle", "services.2.title"). No lodash — the content
 * tree is small and this is the only place that needs path addressing.
 */
...
export function setPath<T>(obj: T, path: string, value: unknown): T {
  ...
  return setAt(obj, keys, value) as T
}
 ...
 * replace wholesale** (so a stored content row's deletes/reorders stick, and
 * a stored array is never element-merged with the default). Keys present in
 * `base` but missing from `override` keep the base value — this is what lets
 * the content schema grow without re-seeding old DB rows.
 */
