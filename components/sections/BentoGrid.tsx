        ...
        renderItem={(p, i) => (
          ...
          >
            ...
            <div className="relative">
              ...
              >
                <IconField path={`pages.home.bentoPillars.${i}.icon`} value={p.icon} size={p.feature ? 26 : 22} strokeWidth={1.6} />
              </span>
              <EditableRichText
                path={`pages.home.bentoPillars.${i}.eyebrow`}
                value={p.eyebrow}
                as="p"
                className={clsx('text-[0.66rem] font-bold tracking-[0.14em] uppercase mb-3', p.feature ? 'text-[var(--accent-light)]' : 'text-[var(--accent-dark)]')}
              />
              <EditableRichText
                path={`pages.home.bentoPillars.${i}.title`}
                value={p.title}
                as="h3"
                className={clsx('font-serif-display font-normal leading-[1.2] mb-3', p.feature ? 'text-[1.7rem] lg:text-[2rem] text-white' : 'text-[1.15rem] text-[var(--ink)]')}
              />
              <EditableRichText
                path={`pages.home.bentoPillars.${i}.body`}
                value={p.body}
                as="p"
                className={clsx('leading-relaxed font-light', p.feature ? 'text-[0.95rem] text-white/70 max-w-md' : 'text-[0.84rem] text-[var(--ink-3)]')}
              />
            </div>
