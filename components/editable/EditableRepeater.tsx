...
export default function EditableRepeater<T>({
  ...
  useEffect(() => {
    ...
    function onDocMouseDown(e: MouseEvent) {
      const el = itemRefs.current[selectedIndex as number]
      if (el && !el.contains(e.target as Node)) setSelectedIndex(null)
    }
    ...
    <>
      {current.map((item, i) => (
        <div
          ...
          className={clsx('relative', itemClassName)}
        ...
        >
          ...
          <div
            className={clsx(
              ...
              selectedIndex === i ? 'outline-[#3b82f6]/40' : 'outline-transparent'
            )}
          >
            {renderItem(item, i)}
          </div>
      ...
      {allowAdd && (
        <button
          type="button"
          onClick={() => commit([...current, newItem()])}
          className={clsx(
